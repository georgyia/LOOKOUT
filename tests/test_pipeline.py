...
def test_analyze_writes_artifacts_and_events(tmp_path: Path) -> None:
    ...
    coverage = analyze(video, out, _fake_stage, AnalysisConfig(target_fps=5.0)).coverage
...
def test_a_participant_with_no_face_still_appears_in_coverage(tmp_path: Path) -> None:
    ...
    coverage = analyze(video, out, _one_slot_only, AnalysisConfig(target_fps=5.0)).coverage
